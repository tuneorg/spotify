...
export class Playlist {
    ...
    public async resolve(Id: string): Promise<UnresolvedData> {
        try {
            ...
            const tracks = playlist.tracks.items.map((item: any) => Spotify.buildUnresolved(item.track));
            let next = playlist.tracks.next;

            while (next) {
                const nextPage = await this.spotify.makeRequest(next.split("v1")[1] as string);
                tracks.push(...nextPage.items.map((item: any) => Spotify.buildUnresolved(item.track)));
                next = nextPage.next;
            }
